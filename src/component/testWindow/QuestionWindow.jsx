...
const OptionCard = ({ option, isSelected, onSelect }) => {
  return (
    <Card
      ...
      className={`inline-flex p-6 border-2 shadow-md transition-all duration-200 items-center min-w-[500px] ${
        isSelected
    ...
    >
      <span
        className={`w-5 h-5 mr-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all duration-200 ${
...
const QuestionWindow = ({
  ...
  // Disable submit button only if selected answer is already submitted
  const submitDisabled = submittedAnswer === selectedAnswer && Boolean(submittedAnswer);
